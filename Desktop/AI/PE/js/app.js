...
const API = {
    ...
    async ensureAvailability() {
        if (CONFIG.USE_API && this._isAvailable) return true;
        if (!CONFIG.API_URL) return false;

        if (!this._availabilityPromise) {
            this._availabilityPromise = this.checkAvailability().then(result => {
                this._isAvailable = result;
                return result;
            });
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        console.warn('Health check falló, intentando continuar:', error);
    }

    // Verificar si hay una sesión activa antes de bloquear
    const hasExistingSession = !!(Utils.storage.get('token') && Utils.storage.get('currentUser'));

    if (CONFIG.ENFORCE_API && !CONFIG.USE_API && !hasExistingSession) {
        // Solo bloquear si no hay sesión previa
    ...
    if (CONFIG.ENFORCE_API && !CONFIG.USE_API && hasExistingSession) {
        // Si hay sesión previa, advertir pero no bloquear
        console.warn('Backend desconectado, pero se permite continuar con sesión existente');
